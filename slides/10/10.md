...
$$𝓛^{(t)}(⇉W_t; →W_{1..(t-1)}) = ∑_i \Big[\ell\big(t_i, y^{(t-1)}(→x_i; ⇉W_{1..(t-1)}) + y_t(→x_i; →W_t)\big)\Big] + \frac{1}{2} λ ||→W_t||^2.$$
...
$γ_t$ which would minimize the loss $∑_i \big[\ell\big(t_i, y^{(t-1)}(→x_i) + γ_t y_t(→x_i)\big)\big] + \frac{1}{2} λ ||→W_t||^2$.
