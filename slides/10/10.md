...
# Gradient Boosting Decision Trees, PCA
...
~~~
In the following text, we drop the parameters or $y^{(t-1)}$ and $y_t$ for
brevity.
...
~~~
and the loss per-example loss is defined as
$$\ell\big(t_i, y(→x_i)\big) = -\log σ\big(y(→x_i)\big)^t_i.$$
...
~~~
and the loss per-example loss is defined analogously as
