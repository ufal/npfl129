...
var Utils = function () {
    ...
    _createClass(Utils, null, [{
        ...
        value: function create_fast_color_scaler(colors, n_shades) {
            ...
            // Changed by Milan Straka to work without Plotly.
            function RGB(css) {
              var bigint = parseInt(css.substr(1), 16);
            ...
            // Precompute n_shades colors
            var table = [new RGB(colors[0])];
            for (var i = 1; i < n_shades - 1; i++) {
              var x = i / (n_shades - 1) * (colors.length - 1);
              var left = Math.floor(x);
              var l = new RGB(colors[left]);
              var r = new RGB(colors[left + 1]);
              var t = x - left;
              table.push(new RGB("#000000"));
              table[i].r = r.r * t + l.r * (1 - t);
              table[i].g = r.g * t + l.g * (1 - t);
              table[i].b = r.b * t + l.b * (1 - t);
            }
            table.push(new RGB(colors[colors.length - 1]));

            // Return the fast lookup
            return function(x) {
              var i = Math.floor((x - domain[0]) / (domain[1] - domain[0]) * n_shades);
              return i < 0 ? table[0] : i >= table.length ? table[table.length - 1] : table[i];
            }
            ...
            // Milan Straka: The original implementation follows
