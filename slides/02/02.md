...
~~~
There are various approaches to minimize it, but for linear regression, an explicit solution exits:
$$→w = (⇉X^T⇉X)^{-1}⇉X^T→t.$$
...
~~~
Note that the $L^2$-regularization is usually not applied on _bias_, only on the
“proper” weights, because we cannot really overfit via bias.
~~~
Also, without penalizing bias, $L^2$-regularization is invariant to shifts
(i.e., adding a constant to all targets would result in the same solution with
only bias increased by that constant; if bias would be penalized, this would not
be true).
...
~~~
Note that the matrix $⇉X^T⇉X + λ⇉I$ is always regular for $λ>0$ (you can show it
is positive definite), so another effect of $L^2$-regularization is that the
...
$⇉U_r^T ⇉U_r$ is a top left submatrix of $⇉U^T ⇉U$. Analogously,
