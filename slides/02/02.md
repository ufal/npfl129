...
$$𝔼_x [αf(x) + βg(x)] = α𝔼_x [f(x)] + β𝔼_x [g(x)].$$
...
~~~
**Bias** of an estimator is the difference between the expected value of the estimator
and the true value being estimated:
$$\textit{estimator~bias} ≝ 𝔼_{\textit{estimate}∼\textrm{estimator}}[\textit{estimate}] - \textit{true estimated value}.$$
...
$𝔼[\textit{estimate}] = 𝔼_P [f(x)]$, which is exactly the true estimated value.
