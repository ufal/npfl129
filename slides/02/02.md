...
- Mathematically describe and implement the **stochastic gradient decent**
  algorithm.
...
only the raw inputs. However, many algorithms can representing
...
- Common solution: normalize the featuers
  ...
  - Standardization: $x_{i,j}^\text{standard} = \frac{ x_{i,j} - \hat{\mu}_{j} }{\hat{\sigma{j}}}$
