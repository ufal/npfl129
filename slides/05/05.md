...
- $$∑_{k=1}^K π(→x)_k = 1,$$
~~~
- $$∀\,j ∈ \{1, 2, …, D\}, ∀\,k ∈ \{1, 2, …, K\}: ∑_{i=1}^N π(→x_i)_k x_{i,j} = ∑_{i=1}^N \Big[t_i == k\Big] x_{i,j}.$$
...
$$\begin{aligned}
  F_1 =& \frac{2}{\mathrm{precision}^{-1} + \mathrm{recall}^{-1}} \\
      =& \frac{2 ⋅ \mathrm{precision} ⋅ \mathrm{recall}}{\mathrm{precision} + \mathrm{recall}}
         = \frac{\mathit{TP~~~~~}+\mathit{~~~~~TP}}{\mathit{TP}+\mathit{FP}+\mathit{TP}+\mathit{FN}}.
\end{aligned}$$
...
$$ F_β = \frac{(1 + β)^2 ⋅ \mathrm{precision} ⋅ \mathrm{recall}}{β^2 ⋅ \mathrm{precision} + \mathrm{recall}}
       = \frac{\hphantom{()}β^2\mathit{TP~~~~~}+\mathit{~~~~~TP}}{β^2(\mathit{TP}+\mathit{FP})+\mathit{TP}+\mathit{FN}}.$$
...
to be _negative_ and the others _different kinds of possitive_. For each of the
...
~~~
- **macro-averaged** $F_1$ (or just **macro** $F_1$): we first compute tje
  the $F_1$-scores of the individual binary classifications and then compute
...
- **true positive rate** or **sensitivity** (probability of detection):
  $\frac{\mathit{TP}}{\textit{target positives}} = \frac{\mathit{TP}}{\textit{TP} + \textit{FN}}$;
- **false positive rate** of **1-specificity** (probability of false alarm):
  $\frac{\mathit{FP}}{\textit{target negatives}} = \frac{\mathit{FP}}{\textit{FP} + \textit{TN}}$;
...
| |Target positive|Target negative| |
|-|---------------|---------------|-|
|Predicted positive| **True Positive** (**TP**) | **False Positive** (**FP**)<br>Type I Error | **precision**<br>$\frac{\mathit{TP}}{\mathit{TP}+\mathit{FP}}$ $\begin{smallmatrix}\colorbox{darkgreen}{\color{red}1}\colorbox{darkgreen}{\phantom{0}}\\\colorbox{gray}{\phantom{0}}\colorbox{gray}{\phantom{0}}\end{smallmatrix}$ |
|Predicted negative| **False Negative** (**FN**)<br>Type II Error | **True Negative** (**TN**) |
|                     | **true positive rate**, **recall**,<br> **sensitivity** $\frac{\mathit{TP}}{\mathit{TP}+\mathit{FN}}$ $\begin{smallmatrix}\colorbox{darkgreen}{\color{red}1}\colorbox{gray}{\phantom{0}}\\\colorbox{darkgreen}{\phantom{0}}\colorbox{gray}{\phantom{0}}\end{smallmatrix}$ | **false positive rate** $\frac{\mathit{FP}}{\mathit{FP}+\mathit{TN}}$ $\begin{smallmatrix}\colorbox{gray}{\phantom{0}}\colorbox{darkgreen}{\color{red}1}\\\colorbox{gray}{\phantom{0}}\colorbox{darkgreen}{\phantom{0}}\end{smallmatrix}$<br>**specificity** $\frac{\mathit{TN}}{\mathit{FP}+\mathit{TN}}$ $\begin{smallmatrix}\colorbox{gray}{\phantom{0}}\colorbox{darkgreen}{\phantom{0}}\\\colorbox{gray}{\phantom{0}}\colorbox{darkgreen}{\color{red}1}\end{smallmatrix}$ |

- $F_1$-score = $\frac{2 ⋅ \mathrm{precision} ⋅ \mathrm{recall}}{\mathrm{precision} + \mathrm{recall}} = \frac{\mathit{TP~~~~}+\mathit{~~~~TP}}{\mathit{TP}+\mathit{FP}+\mathit{TP}+\mathit{FN}}$ $\begin{smallmatrix}\colorbox{darkgreen}{\color{red}2}\colorbox{darkgreen}{\phantom{0}}\\\colorbox{darkgreen}{\phantom{0}}\colorbox{gray}{\phantom{0}}\end{smallmatrix}$

- accuracy = $\frac{\mathit{TP}+\mathit{TN}}{\mathit{TP}+\mathit{FP}+\mathit{FN}+\mathit{TN}}$ $\begin{smallmatrix}\colorbox{darkgreen}{\color{red}1}\colorbox{darkgreen}{\phantom{0}}\\\colorbox{darkgreen}{\phantom{0}}\colorbox{darkgreen}{\color{red}1}\end{smallmatrix}$
