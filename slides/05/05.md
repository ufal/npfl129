...
~~~
![w=40%,f=left](lagrange_example.svgz)
...
<div style="float: right; width: 26%">![w=95%,h=center](lagrange_example.svgz)<br>![w=100%](lagrange_equalities.svgz)</div>
...
$$𝓛 = \bigg(∑_i p_i \log p_i\bigg) - λ\bigg(∑_i p_i - 1\bigg).$$

~~~
Computing the derivative with respect to $p_i$ and setting it equal to zero, we get
$$0 = \frac{∂𝓛}{∂p_i} = 1⋅\log(p_i) + p_i ⋅ \frac{1}{p_i} - λ = \log(p_i) + 1 - λ.$$
...
$$𝓛 = -\textcolor{red}{H[p]} - \textcolor{blue}{λ_1 \Big(∫p(x) \d x - 1\Big)}
      -\textcolor{darkgreen}{λ_2 \big(𝔼[x] - μ\big)} - \textcolor{magenta}{λ_3\big(\Var(x) - σ^2\big)}.$$

~~~
By expanding all definitions to integrals, we get
$$\begin{aligned}
𝓛(p(x), x, →λ; μ, σ^2) =& ∫\Big(\textcolor{red}{p(x)\log p(x)} - \textcolor{blue}{λ_1 p(x)}
                          -\textcolor{darkgreen}{λ_2 p(x) x} - \textcolor{magenta}{λ_3 p(x) (x - μ)^2}\Big) \d x \\
                        & +\textcolor{blue}{λ_1} + \textcolor{darkgreen}{μ λ_2} + \textcolor{magenta}{σ^2 λ_3}.
\end{aligned}$$

~~~
We now set the derivative of the inner of the integral with respect to $p(x)$ equal to zero:
$$0 = \textcolor{red}{1 + \log p(x)} - \textcolor{blue}{λ_1} - \textcolor{darkgreen}{λ_2 x} - \textcolor{magenta}{λ_3 (x - μ)^2},$$
