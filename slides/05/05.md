...
We now compute partial derivatives of the Lagrangian (analogously to the
functional derivatives), notably the values
$$\frac{∂}{∂π(→x_i)_j}L.$$
...
~~~
$$\begin{aligned}
φ(→x)^T φ(→z) =& 1 + ∑_i x_i z_i + ∑_{i,j} x_i x_j z_i x_j + ∑_{i,j,k} x_i x_j x_k z_i z_j z_k \\
...
~~~
There is quite a lot of theory behind kernel construction. In machine learning,
the most often used kernels are:
...
Assume we have a dataset $⇉X ∈ ℝ^{N×D}$, $→t ∈ ℝ^N$, feature map $φ$ and model
$$y(→x) ≝ →φ(→x)^T →w + b.$$
...
$$\argmax_{w,b} \frac{1}{||→w||} \min_i \big[t_i (→φ(→x)^T →w + b)\big]$$
to
$$\argmin_{w,b} \frac{1}{2} ||→w||\textrm{~~given that~~}t_i y(→x_i) ≥ 1.$$
...
$$L = \frac{1}{2} ||→w|| - ∑_i a_i \big[t_i y(→x_i) - 1\big].$$
...
$$\argmin_{w,b} C ∑_i ξ_i + \frac{1}{2} ||→w||\textrm{~~given that~~}t_i y(→x_i) ≥ 1 - ξ_i\textrm{~and~}ξ_i ≥ 0.$$
...
and also $→μ=(μ_1, …, μ_N)$:
$$L = \frac{1}{2} ||→w|| + C ∑_i ξ_i - ∑_i a_i \big[t_i y(→x_i) - 1 + ξ_i\big] - ∑_i μ_i ξ_i.$$

~~~
Solving for the critical points and substituting for $→w$, $b$ and $→ξ$, we obtain the Lagrangian in the form
$$L = ∑_i a_i -  \frac{1}{2} ∑_i ∑_j a_i a_j t_i t_j K(→x_i, →x_j),$$
which is identical to the previous case, but the constraints are a bit
different: $∀_i: C ≥ a_i ≥ 0$ and $∑_i a_i t_i = 0$.
...
![w=40%,h=center](svm_softmargin_supports.pdf)

---
# SGD Formulation of Primary SVM

TODO

---
# Solving Dual SVM Optimization Problem
