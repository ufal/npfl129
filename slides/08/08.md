...
$$→w_\mathrm{MAP} = \argmin_{→w} ∑\nolimits_{i=1}^N -\log p(→x_i | →w) {\color{gray} - \frac{c}{2} \log(2πσ^2)} + \frac{\|→w\|^2}{2σ^2},$$
...
~~~
- In order to use Bernoulli NB, the features _must_ be binary. However, a
