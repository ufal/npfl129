...
$$\textcolor{red}{H(Y)} - \textcolor{blue}{H(Y | X)}
  ...
  = 𝔼_{x,y} \left[\frac{\textcolor{blue}{P(x, y)}}{\textcolor{blue}{P(x)} \textcolor{red}{P(y)}}\right].$$
...
$$I(X; Y) = 𝔼_{x,y} \left[\frac{P(x, y)}{P(x)P(y)}\right].$$
...
~~~
In _Bayesian probability_ interpretation, probability is instead a quantification
of uncertainty. Bayesian probability can be considered an extension of
...
$→w$. It is therefore **likelihood**, because it is a function of $⇉w$.
...
prior and likelihood, the posterior distribution family is the same as the
prior. Such a distribution is called a **conjugate prior** of a given likelihood
function.
...
~~~
Therefore, instead of modeling $p(C_k|→x)$ we model the prior $p(C_k)$ and the
distribution $p(→x | C_k)$.
...
to avoid too sharp distributions (in Scikit-learn, the default value of $α$ in Sikit-learn is $10^{-9}$ times the largest variance
  of all features).
...
$$p_{d,k}
  ...
  \textrm{ where $λ$ is set to fulfil }∑\nolimits_d p_{d,k} = 1.$$
...
and instead use
$$p_{d, k} = \frac{n_{d, k} + α}{∑_j n_{d, k} + αD}$$
...
- To use multinomial NB, the features should roughly follow the multinomial
  distribution – i.e., they must be non-negative, “compete” with each other,
  and higher values should be exponentially less likely.
~~~
...
| What's<br>learned | Decision boundary | Probability distributio of the data |
