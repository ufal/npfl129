...
For neural network models, training models with independent initializations is
usually enough, given that the loss has many local minima, so the models tend to
be quite independent just from using different initializations.
...
~~~
For each leaf, our model predicts the average of the training examples belonging
to that leaf, $t̂_𝓣 = \frac{1}{|I_𝓣|} ∑_{i ∈ I_𝓣} t_i$.

~~~
We use a **criterion** $c_𝓣$ telling us how _uniform_ or _homogeneous_ the
training examples of node $𝓣$ are
~~~
