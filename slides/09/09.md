...
$\displaystyle 𝓛(p_𝓣) = ∑_{i ∈ I_𝓣} (p_𝓣 - t_i)^2$
~~~
$= n_𝓣(0) \textcolor{blue}{(p_𝓣 - 0)^2} + n_𝓣(1) \textcolor{magenta}{(p_𝓣 - 1)^2}$

~~~
$\displaystyle \phantom{𝓛(p_𝓣)}
...
~~~
$\displaystyle \phantom{𝓛(p_𝓣)} = \big(n_𝓣(0) + n_𝓣(1)\big) \textcolor{green}{(1-p_𝓣)} \textcolor{red}{p_𝓣} = |I_𝓣| p_𝓣 (1-p_𝓣).$
...
a Lagrangian with constraint $∑_k p_k = 1$),
we get that the $→p$ minimizing the loss fulfils $p_k = p_𝓣(k)$.

~~~
The value of the loss with respect to $p_𝓣$ is then

$\displaystyle \kern10em\mathllap{𝓛(p_𝓣)} = ∑_{i ∈ I_𝓣} - \log p_{t_i}$
