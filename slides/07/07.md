...
$$\frac{∂^2L}{∂a_j^2} = 2K(→x_i, →x_i) - K(→x_i, →x_i) - K(→x_j, →x_j).$$
...
for $0 < a_j < C$ it must hold that $t_j y(→x_j) = 1$. Combining it with
with the equation $b = E_j + t_j - ∑_l a_l t_l K(→x_j, →x_l)$, we get the following
value
$$b_j = b - E_j - t_i (a_i - a_i^\textrm{old})K(→x_i, →x_j) - t_j (a_j - a_j^\textrm{old})K(→x_j, →x_j).$$

~~~
Analogously for 0 < a_i < C we get
$$b_i = b - E_j - t_i (a_i - a_i^\textrm{old})K(→x_i, →x_j) - t_j (a_j - a_j^\textrm{old})K(→x_j, →x_j).$$

~~~
Finally, if $a_j, a_i ∈ \{0, C\}$, we know that all values between $b_i$ and $b_j$ fulfil the KKT conditions.
We therefore arrive at the following update for bias:
$$b^\textrm{new} = \begin{cases}
  b_i & \textrm{if~~} 0 < a_i < C \\
  b_j & \textrm{if~~} 0 < a_j < C \\
  \frac{b_i + b_j}{2} & \textrm{otherwise}.
\end{cases}$$

---
section: DecisionTreeRefresh
# Decision Trees
