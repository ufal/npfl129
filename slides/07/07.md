...
For $E_j ≝ y(→x_j) - t_j$, we get
$$a_j ← a_j - t_j\frac{E_i - E_j}{2K(→x_i, →x_j) - K(→x_i, →x_i) - K(→x_j, →x_j)},$$
~~~
which we clip to $[H, L]$ range with
$$\begin{aligned}
...
~~~
When we set
$$a_i ← a_i + t_i t_j\big(a_j^\textrm{old} - a_j).$$
...
We define
$$\begin{aligned}
b_i &= b - E_i - t_i (a_i - a_i^\textrm{old})K(→x_i, →x_i) - t_j (a_j - a_j^\textrm{old})K(→x_i, →x_j) \\
