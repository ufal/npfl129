...
Now, we need to compute the denomiator $P(\text{pos})$ by splitting into joint probabilities:

$
P(\text{pos}, \text{def}) + P(\text{pos}, \neg\text{def}) = P(\text{pos}|\text{def})P(\text{def}) + (1 - \underbrace{P(\neg\text{pos} | \neg\text{def})}_\text{specificity})(1 - P(\neg{def}))
$
...
$$P(\text{def}|\text{pos}) = \frac{.99 \cdot 10^{-5}}{.99 \cdot 10^{-5} + (1 - .99)(1 - 10^{-5})} = 0.98\%

---
class: center
# Textbook Example: How accurate do we need to be?

![w=45%](defect_proba.svgz) ![w=28%](xkcd_bayes_theorem.png)
...
| Illustration | ![w=60%](model_discriminative.png) | ![w=60%](model_generative.png) |

---
# Generative and Discriminative Models: Remarks

- Big topic in 2000's: Generative models better with very little data, with enough data discriminative models are always better.
