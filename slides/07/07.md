  ...
  - **for** $i$ in $1, 2, …, N$:
    ...
    - **if** ($a_i < C-\mathit{tol}$ **and** $t_i E_i < -\mathit{tol}$) **or** ($a_i > \mathit{tol}$ **and** $t_i E_i > \mathit{tol}$):
      ...
      - try updating $a_i$, $a_j$ to maximize $𝓛(a_1, a_2, …, a_N)$ such that
        $0 ≤ a_k ≤ C$ and $∑_i a_i t_i = 0$; if successful, set $b$ fo fulfill
...
~~~
Finally, if $a_j^\textrm{new}, a_i^\textrm{new} ∈ \{0, C\}$, we know that all
values between $b_i^\textrm{new}$ and $b_j^\textrm{new}$ fulfill the KKT
...
space recieve tied votes (and such regions are then ambiguous).
