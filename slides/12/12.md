...
2D, ellipsoids in 3D) centered at the mean $→μ$ with size of the axes depending
on the corresponding diagonal entries in the covariance matrix.
...
prove that its increase also causes the $\log p(⇉X; →w)$ to increase.
