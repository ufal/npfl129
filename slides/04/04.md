...
|------|------------|--------------|------|----------|
| linear regression | identity | ? | $\textrm{MSE} ∝ ∑ (y(→x) - t)^2$ | $\big(y(→x) - t\big) ⋅ →x$ |
~~~
| logistic regression | $σ(x)$ | Bernoulli | $\textrm{NLL} ∝ ∑ -\log(p(t \vert →x))$ | ? |
~~~ ~~
| logistic regression | $σ(x)$ | Bernoulli | $\textrm{NLL} ∝ ∑ -\log(p(t \vert →x))$ | $\color{red}\big(a(y(→x)) - t\big) ⋅ →x$ |
...
| logistic regression | $σ(x)$ | Bernoulli | $\textrm{NLL} ∝ ∑_i -\log(p(t \vert →x))$ | $\big(a(y(→x)) - t\big) ⋅ →x$ |
...
$$p(C_i | →x; ⇉W) = \softmax(⇉W →x)_i = \frac{e^{⇉W_i^T →x}}{∑_j e^{⇉W_j^T →x}}.$$
...
| logistic regression | $σ(x)$ | Bernoulli | $\textrm{NLL} ∝ ∑ -\log(p(t \vert →x))$ | $\big(a(y(→x)) - t\big) ⋅ →x$ |
| multiclass<br>logistic regression | $\small\operatorname{softmax}(x)$ | categorical | $\textrm{NLL} ∝ ∑ -\log(p(t \vert →x))$ | $\big(a(y(→x)) - t\big) ⋅ →x$ |
...
| logistic regression | $σ(x)$ | Bernoulli | $\textrm{NLL} ∝ ∑ -\log(p(t \vert →x))$ | $\big(a(y(→x)) - t\big) ⋅ →x$ |
| multiclass<br>logistic regression | $\small\operatorname{softmax}(x)$ | categorical | $\textrm{NLL} ∝ ∑ -\log(p(t \vert →x))$ | $\big(a(y(→x)) - t\big) ⋅ →x$ |
| Poisson regression | $\small\exp(x)$ | Poisson | $\textrm{NLL} ∝ ∑ -\log(p(t \vert →x))$ | $\big(a(y(→x)) - t\big) ⋅ →x$ |
