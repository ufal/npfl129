...
1. **regression**: The goal of regression is to predict a real-valued target
...
unseen_ data, to achieve the lowest **generalization error** or **test error**. We
...
Let $f : ℝ → ℝ$ be a function. If it has a minimum (or maximum) in $x$
...
Let $f : ℝ^D → ℝ$ be a function. If it has a minimum (or maximum) in
...
SVD, which will be demonstrated in the next lecture.
