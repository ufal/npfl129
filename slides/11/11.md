...
$→X \in \mathbb{R}^{n \times m}$ and $→X_k = \sigma_1 →u_1 →v_1^T + \ldots + \sigma_k →u_k →v_k^T$ its approximation using SVD. For each $→B$
...
~~~
$$⇉A^k→v = λ_1^k a_1→u_2 + λ_2^k a_2→u_2 + λ_3^k a_3→u_3 … = \lambda_1^k \left(a_1→u_1 + \frac{\lambda_2^k}{\lambda_1^k}a_2→u_2 + \frac{\lambda_3^k}{\lambda_1^k}u_3→u_3 + \ldots\right)$$
