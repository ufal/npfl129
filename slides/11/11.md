...
Let $⇉A ∈ ℂ^{N×M}$ be an $N×M$ matrix.

~~~
- A vector $→v ∈ ℂ^M$ is a (right) **eigenvector**, if there exists an
  **eigenvalue** $λ ∈ ℂ$,
  such that
  $$⇉A →v = λ →v.$$

~~~
- If $⇉A ∈ ℝ^{N×N}$ is a real symmetric matrix, than it has $N$ real
...
because if $⇉A = ⇉V^T ⇉Λ ⇉V$, then for $⇉Λ^{1/2} ⇉V$ we get
$$(⇉Λ^{1/2} ⇉V)^T ⇉Λ^{1/2} ⇉V = ⇉V^T ⇉Λ^{1/2} ⇉Λ^{1/2} ⇉V = ⇉V^T ⇉Λ ⇉V = ⇉A.$$
...
It can be proven that if we construct a MLP _autoencoder_,
...
eigenvalue with absolute value strictly larger than absolute value of all other
eigenvalues) and the corresponding eigenvector (it is used for example to
compute PageRank). It works as follows:

<div class="algorithm">

**Input**: Matrix $⇉A$ with a dominant eigenvalue.<br>
...
~~~
- mutiplying it by other eigenvectors $→v_i$ gives the same result as
  multiplying $⇉A$:
...
eigenvalues, except for $→v_1, λ_1$.
...
~~~
When clustering documents, we usually normalize TF-IDF normalized so that each
feature vector has length 1 (i.e., L2 normalization), because then
