...
When we apply SVD on mean-centered data $→X - →\mu$ (where $→\mu$ is row-wise average of $→X$), singular values get a new interpretation: **components explaining variability in the data**.

$$
||→X - →x||_F^2 =
\operatorname{trace}\underbrace{\left((→X - →\mu)^T(→X - →\mu)\right)}_{N\operatorname{Cov}(→X)}  =
...
Let $→S = \frac{1}{N}(→X - →\mu)^T(→X - →\mu)$, then PCA of $→X$ are the eigenvectors of $→S$, i.e., the $→V$ matrix of the SVD decomposition of $→X - →\mu$.
...
~~~
  - $⇉S ← ⇉S - λ_i →v_i →v_i^T$
~~~
- Return $(⇉X - →μ) ⇉V$, where the columns of $⇉V$ are $→v_1, →v_2, …, →v_M$.
</div>
