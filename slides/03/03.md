...
depending whether $f(→x; →w) = →x^T→w + b$ is smaller/larger than a given threshold.
...
- Consider two points on the decision boundary. Because $f(→x_1; →w)=f(→x_2; →w)$,
...
for all train data,
$$\operatorname{sign}(f(→x_i; →w)) = \operatorname{sign}(→w^T →x_i) = t_i,$$
or equivalently,
$$t_i f(→x_i; →w) = t_i →w^T →x_i > 0.$$
...
→w_\mathrm{ML} &= \argmax_→w p_\textrm{model}(\mathbb X; →w) \\
               ...
               &= \argmin\nolimits_→w D_\textrm{KL}(p̂_\textrm{data}||p_\textrm{model}(→x; →w)) \color{gray} + H(p̂_\textrm{data})
\end{aligned}$$
