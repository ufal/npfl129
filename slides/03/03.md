...
for all train data,
$$\operatorname{sign}(y(→x_i; →w)) = \operatorname{sign}(→x_i^T →w) = t_i,$$
or equivalently,
...
$$\begin{aligned}
→w_\mathrm{MLE} &= \argmax_→w p_\textrm{model}(\mathbb X; →w) \\
...
$$\begin{aligned}
→w_\mathrm{MLE} &= \argmax_→w p_\textrm{model}(\mathbb T | \mathbb X; →w) \\
