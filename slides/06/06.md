...
~~~
The SGD update of a linear regression using a minibatch of examples $𝔹$ is then
$$→w ← →w - \frac{α}{|𝔹|}∑\nolimits_{i ∈ 𝔹}\big(φ(→x_i)^T →w - t_i\big) φ(→x_i).$$
...
of the $N$ input feature vectors $φ(→x_i)$.

~~~
By induction, we can start with $→w = →0 = ∑_{i=1}^N 0 ⋅ φ(→x_i)$.
~~~
Assuming $→w = ∑_{i=1}^N β_i ⋅ φ(→x_i)$,
after an SGD update we get
$$\begin{aligned}
→w &← →w - \frac{α}{|𝔹|}∑_{i ∈ 𝔹} \big(φ(→x_i)^T →w - t_i\big) φ(→x_i)\\
   &← ∑\nolimits_{i=1}^N \Big(β_i - \big[i ∈ 𝔹\big] ⋅ \frac{α}{|𝔹|} \big(φ(→x_i)^T →w - t_i\big)\Big) φ(→x_i).
\end{aligned}$$

~~~
Every $β_i$ for $i ∈ 𝔹$ changes to $β_i - \frac{α}{|𝔹|}\big(φ(→x_i)^T →w - t_i\big)$, so after
substituting for $→w$ we get
$$β_i ← β_i - \frac{α}{|𝔹|}\bigg(\Big(∑\nolimits_{j=1}^N β_j φ(→x_i)^T φ(→x_j)\Big) - t_i\bigg).$$

---
# Visualization of the Primal and Dual Representations

![w=100%,v=middle](dual_formulation.svgz)
...
- until convergence (or patience runs out), process a minibatch of examples $𝔹$:
  - simultaneously for all $i ∈ 𝔹$ (the $β_j$ on the right side must
    not be modified during the batch update):
    - $β_i ← β_i - \frac{α}{|𝔹|}\Big(\big(∑\nolimits_{j=1}^N β_j K_{i,j}\big) - t_i\Big)$
</div>

~~~
The predictions for input $→z$ are then performed by computing
$$y(→z) = φ(→z)^T →w = ∑\nolimits_{i=1}^N β_i →φ(→z)^T →φ(→x_i).$$
...
the predictions to
$$y(→z) = φ(→z)^T →w + b = \Big(∑\nolimits_{i=1}^N β_i →φ(→z)^T →φ(→x_i)\Big) + b$$
and update the bias $b$ separately.

~~~
To update the bias, we use SGD, and the resulting update is
$$b ← b - \frac{α}{|𝔹|}∑\nolimits_{i ∈ 𝔹}\bigg(\Big(∑\nolimits_{j=1}^N β_j K_{i,j}\Big) + b - t_i\bigg).$$
...
examples more strongly. We could interpret it as an extended version of the
...
$$\frac{|y(→x_i)|}{\|→w\|}
  \xlongequal{\substack{\textrm{assuming~}y\textrm{~classifies}\\\textrm{all~}→x_i\textrm{~correctly}}}
  \frac{t_i y(→x_i)}{\|→w\|}.$$
...
partial derivatives and that $∇_{→x}g(→x) ≠ 0$.
...
It is easy to verify that if $𝓛(→x, λ)$ has a minimum in $→x$ and $λ$ fulfills
the KKT conditions $g(→x) ≥ 0, λ ≥ 0, λ g(→x) = 0,$
the Lagrangian $𝓛$ has a **maximum** in $λ$ subject to $λ ≥ 0$:
~~~
...
~~~
On the other hand, if $𝓛(→x, λ)$ has a minimum in $→x$, and a maximum
