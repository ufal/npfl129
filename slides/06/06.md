...
~~~
- **Polynomial kernel or degree $d$**, also called _homogenous polynomial kernel_
  $$K(→x, →z) = (γ →x^T→z)^d,$$
...
- **Polynomial kernel or degree at most $d$**, also calles _nonhomogenous
  polynomial kernel_
  $$K(→x, →z) = (γ →x^T→z + 1)^d,$$
...
we only need to keep the points on the boundary, the so-called **support vectors**.
Even though SVM is a nonparametric model, it stores only a subset of data.
